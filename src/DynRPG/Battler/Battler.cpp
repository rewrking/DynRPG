 ...
 *
 *****************************************************************************/
void Battler::setRow(BattleRow newRow)
{
	rowAlt = newRow;
	row = newRow;
}
...
bool Battler::executeAction(bool skipPluginHandlers)
{
	...
	if (skipPluginHandlers)
	{
		asm volatile("call *%%esi"
					 : "=a"(ret), "=d"(_edx)
					 : "S"(0x49A04C), "a"(**sceneObjects[static_cast<uchar>(Scene::Battle)]), "d"(this)
	...
	else
	{
		asm volatile("call *%%esi"
					 : "=a"(ret), "=d"(_edx)
					 : "S"(0x45CDF4), "a"(**sceneObjects[static_cast<uchar>(Scene::Battle)]), "d"(this)
 ...
 *
 *****************************************************************************/
int Battler::getAttributeResist(int id)
{
	int out;
	int tmp;
	int tmp2;
 ...
 *
 *****************************************************************************/
int Battler::getAttackPower(Battler* target)
{
	int power;
	asm volatile("call *%%esi"
				 : "=a"(power), "=d"(_edx)
 ...
 *
 *****************************************************************************/
int Battler::getSkillPower(int skillId, Battler* target)
{
	int power;
	asm volatile("call *%%esi"
