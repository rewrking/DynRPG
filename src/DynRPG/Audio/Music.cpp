...
void Music::play()
{
	asm volatile("call *%%esi"
				 ...
				 : "S"(0x48B408), "a"(system), "d"(this), "c"(0)
				 : "cc", "memory"); // CallHarmonyPlayMusic
	if (system->currentBGM)
		system->currentBGM->set(*this);
	if (system->scene == Scene::Map && system->mapBGM)
		system->mapBGM->set(*this);
}
...
void Music::fadeOut(int time)
{
	asm volatile("call *%%esi"
				 : "=a"(_eax), "=d"(_edx)
				 : "S"(0x48B4E0), "a"(system), "d"(time)
				 : "ecx", "cc", "memory"); // CallHarmonyFadeOutMusic
}
