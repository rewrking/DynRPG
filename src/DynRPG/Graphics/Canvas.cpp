 ...
 *
 *****************************************************************************/
ushort& Canvas::pixel(int x, int y)
{
 ...
 *
 *****************************************************************************/
ushort* Canvas::getScanline(int y)
{
	ushort* ret;
	asm volatile("call *%%esi"
				 : "=a"(ret), "=d"(_edx)
				 : "S"(0x4683B8), "a"(this), "d"(y)
				 : "ecx", "cc", "memory"); // GetAuroraBoardPixelLineAddress
	return ret;
}
...
int Canvas::width()
{
	...
	asm volatile("call *%%esi"
				 : "=a"(ret)
				 : "S"(0x4680CC), "a"(this)
				 : "edx", "ecx", "cc", "memory"); // GetBoardBitmapWidth
	return ret;
}
...
int Canvas::height()
{
	...
	asm volatile("call *%%esi"
				 : "=a"(ret)
				 : "S"(0x4680D8), "a"(this)
				 : "edx", "ecx", "cc", "memory"); // GetBoardBitmapHeight
	return ret;
}
...
void Canvas::draw(int x, int y, Image* image, int srcX, int srcY, int srcWidth, int srcHeight)
{
	...
	asm volatile(
		...
		: "S"(0x4684C0), "a"(this), "d"(x), "c"(y), "m"(image), "D"(&rc)
		: "cc", "memory");
...
void Canvas::drawStretched(int x, int y, int width, int height, Image* image, int srcX, int srcY, int srcWidth, int srcHeight)
{
	...
	asm volatile(
		...
		"call *%%esi;"
		: "=a"(_eax), "=d"(_edx), "=c"(_ecx)
		: "S"(0x4688B0), "a"(this), "d"(&rc2), "c"(image), "D"(&rc1)
		: "cc", "memory");
