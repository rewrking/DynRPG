	...
	PictureEffect effect, int effectStrength)
{
	...
	asm volatile(
		...
		: "S"(0x4C1964), "a"(this), "d"(s.str), "c"(x), "m"(y), "m"(moveWithMap), "m"(magnification), "m"(transparency), "m"(transparency2), "m"(useMaskColor), "m"(red), "m"(green), "m"(blue), "m"(chroma), "m"(effect), "m"(effectStrength)
		: "cc", "memory");
}
	...
	int duration)
{
	asm volatile(
		...
		"call *%%esi"
		: "=a"(_eax), "=d"(_edx), "=c"(_ecx)
		: "S"(0x4C1AD8), "a"(this), "d"(x), "c"(y), "m"(magnification), "m"(transparency), "m"(transparency2), "m"(red), "m"(green), "m"(blue), "m"(chroma), "m"(effect), "m"(effectStrength), "m"(duration)
		: "cc", "memory");
}
