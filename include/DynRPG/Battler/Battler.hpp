#ifndef RPG_BATTLER_HPP
#define RPG_BATTLER_HPP
...
if(battler->animationId == 9) battler->animationId = 0;
	...
	*/
	...
	int destinationY; //!< Destination Y coordinate (used when an actor moves to attack)
		int _unknown_6C;
	BattleRow rowAlt; //!< Internally used, I do not understand the difference from \c row yet
	BattleRow row; //!< Current row (no visible effect on monsters)
	...
	*/
	int getAttackPower(Battler* target);
	...
	/*! \brief Built-in RM2k3 function that returns the strength of a skill on a certain target,
		...
		\return The damage value (again, not final!)
	*/
	int getSkillPower(int skillId, Battler* target);
};

}

#endif // RPG_BATTLER_HPP
