#ifndef RPG_IMAGE_HPP
#define RPG_IMAGE_HPP
...
{
	void** vTable;
	uchar* pixels; //!< Pointer to direct pixel data (stored from top to bottom). Each pixel contains the palette ID (so one could reference it as image->palette[image->pixels[i]] to get the color of a pixel at the location in GGBBRR order)
	...
	*/
	void draw(int x, int y, uchar* newPixels, int srcWidth, int srcHeight, int srcLineWidth, int maskColor = 0);
	...
	/*! \brief Draws another image (or a part of it) onto the image
		...
		RPG::MASK_NONE)
		\note Please also see the \ref optimization section!
		\sa draw(int, int, uchar *, int, int, int, int)
	*/
	...
	*/
	...
	/*! \brief Broken as of v 0.20. Use RPG::Image::drawString instead
		as it is using the same rm2k3 function, but references it in
		memory correctly.
		\sa drawString
	*/
