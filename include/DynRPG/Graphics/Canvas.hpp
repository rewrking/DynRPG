#ifndef RPG_CANVAS_HPP
#define RPG_CANVAS_HPP
...
struct Canvas
{
	...
	*/
	void draw(int x, int y, Image* image, int srcX = 0, int srcY = 0, int srcWidth = -1, int srcHeight = -1);
	...
	/*! \brief Draws an RPG::Image or a part of it onto the canvas,
		...
		\sa RPG::Canvas::draw
	*/
	void drawStretched(int x, int y, int width, int height, Image* image, int srcX = 0, int srcY = 0, int srcWidth = -1, int srcHeight = -1);
	...
	/*! \brief Draws an RPG::Image or a part of it onto the canvas,
		...
		\sa RPG::Canvas::draw
	*/
	void drawCenteredZoomed(int x, int y, double zoomX, double zoomY, Image* image, int srcX = 0, int srcY = 0, int srcWidth = -1, int srcHeight = -1);
	...
	*/
	static inline uint convert16To24Bit(ushort pixel16)
	{
		uint ret = static_cast<uint>(pixel16 & 0xF800) >> 8;
		ret |= static_cast<uint>(pixel16 & 0xE000) >> 13;
		ret |= static_cast<uint>(pixel16 & 0x7E0) << 5;
		ret |= static_cast<uint>(pixel16 & 0x600) >> 1;
		ret |= static_cast<uint>(pixel16 & 0x1F) << 19;
		ret |= static_cast<uint>(pixel16 & 0x1C) << 14;
		return ret;
	}
	...
	*/
	static inline uint convert16To32Bit(ushort pixel16)
	{
	...
	*/
	static inline ushort convert24To16Bit(uint pixel24)
	{
		ushort ret = static_cast<ushort>(pixel24 & 0xF8) << 8;
		ret |= static_cast<uint>(pixel24 & 0xFC00) >> 5;
		ret |= static_cast<uint>(pixel24 & 0xF80000) >> 19;
		return ret;
	}
	...
	/*! \brief Converts a 32-bit pixel value to 16 bit by combining it
		...
		\return 16-bit pixel value (\c RRRRRGGGGGGBBBBB)
	*/
	static inline ushort convert32To16Bit(uint pixel32, ushort prevPixel16)
	{
		// 16 = RRRRRGGGGGGBBBBB
		// 32 = AAAAAAAABBBBBBBBGGGGGGGGRRRRRRRR
		uchar alpha = static_cast<uchar>(pixel32 >> 24);
		if (!alpha)
			return convert24To16Bit(pixel32);
#define c(v1, v2) ((v1) * (alpha + 1) + (v2) * (256 - alpha))
		ushort ret = static_cast<ushort>(c(static_cast<uchar>(pixel32), static_cast<uchar>(prevPixel16 << 3))) >> 3;
		ret |= static_cast<ushort>(c(static_cast<uchar>(pixel32 >> 8), static_cast<uchar>((prevPixel16 & 0x7E0) >> 3)) & 0xFC) << 3;
		ret |= static_cast<ushort>(c(static_cast<uchar>(pixel32 >> 16), static_cast<uchar>((prevPixel16 & 0xF800) >> 8)) & 0xF8) << 8;
#undef c
		return ret;
	}
};
